...
export default function ProfitMatrix({ token, refreshKey }: { token: string; refreshKey?: number }) {
  ...
  // Calculate maintenance fee based on circle size
  const getMaintenanceFee = (circleSize: number) => {
    return 500 * Math.ceil(circleSize / 2);
  };

  // Get the fee scale display text
  const getFeeScaleDisplay = (circleSize: number) => {
    const fee = getMaintenanceFee(circleSize);
    return `${circleSize} members: ${naira(fee)} per winner`;
  };

  // Use dynamic values from circle.currentMonth
  const payoutData = metrics?.circle?.currentMonth;
  const circleSize = payoutData?.circleSize || metrics?.circle?.baselineSize || 0;
  const paidCount = payoutData?.paidMemberCount || 0;
  const grossPayout = payoutData?.grossPayoutPerRecipient || 0;
  const maintenanceFee = payoutData?.maintenanceFeePerRecipient || getMaintenanceFee(circleSize);
  const netPayout = payoutData?.netPayoutPerRecipient || (grossPayout - maintenanceFee);
  const recipientCount = payoutData?.recipientCount || metrics?.circle?.recipientCount || 2;
  const hasEnoughPaidMembers = paidCount >= recipientCount;
  const canCalculatePayout = paidCount > 0 && hasEnoughPaidMembers;
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        ...
        <div className="rounded-xl bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 px-5 py-3 text-right min-w-[200px]">
          <p className="text-xs font-bold text-red-700 dark:text-red-400 uppercase tracking-wide">Payout per recipient</p>
          {loading ? (
            <div className="h-7 w-24 bg-red-200/50 dark:bg-red-800/50 animate-pulse rounded mt-1 mx-auto"></div>
          ) : canCalculatePayout ? (
            <>
              <b className="text-xl text-red-700 dark:text-red-400">
                {naira(Math.round(netPayout))}
              </b>
              <p className="text-[11px] text-red-700 dark:text-red-400 mt-0.5">Net, after maintenance fee</p>
              {circleSize > 0 && (
                <p className="text-[10px] text-red-600 dark:text-red-400 mt-1">
                  {recipientCount} recipient{recipientCount > 1 ? 's' : ''} · Fee: {naira(maintenanceFee)}
                  <br />
                  <span className="text-[9px] text-red-500/80 dark:text-red-400/70">{getFeeScaleDisplay(circleSize)}</span>
                </p>
              )}
            </>
          ) : paidCount === 0 ? (
            <>
              <b className="text-xl text-red-700 dark:text-red-400">—</b>
              <p className="text-[11px] text-red-700 dark:text-red-400 mt-0.5">No members have paid this month</p>
              <p className="text-[10px] text-red-600 dark:text-red-400 mt-1">
                {recipientCount} recipient{recipientCount > 1 ? 's' : ''} required for draw
              </p>
            </>
          ) : (
            <>
              ...
              <p className="text-[11px] text-red-700 dark:text-red-400 mt-0.5">
                Need {recipientCount - paidCount} more paid member{recipientCount - paidCount > 1 ? 's' : ''}
              </p>
              <p className="text-[10px] text-red-600 dark:text-red-400 mt-1">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl p-5 shadow-sm bg-blue-800 text-white">
          ...
          {loading ? (
            <div className="h-8 w-32 bg-blue-600/50 animate-pulse rounded mt-1"></div>
          ) : (
          ...
          <p className="text-blue-200 text-xs mt-3 border-t border-blue-600 pt-3">
            ...
            {paidCount > 0 && ` = ${naira(paidCount * 10000)}`}
          </p>
        </div>

        {/* Owambe Fund */}
        <div className="rounded-2xl p-5 shadow-sm bg-white dark:bg-slate-900 border-2 border-red-600">
          <p className="text-sm font-semibold text-red-700 dark:text-red-400">Owambe / Get-Together Fund</p>
          {loading ? (
            <div className="h-8 w-32 bg-slate-200 dark:bg-slate-700 animate-pulse rounded mt-1"></div>
          ) : (
          ...
          <p className="text-slate-600 dark:text-slate-400 text-xs mt-3 border-t dark:border-slate-700 pt-3">
            ...
            {paidCount > 0 && ` = ${naira(paidCount * 1000)}`}
          </p>
        </div>

        {/* Late Penalties */}
        <div className="rounded-2xl p-5 shadow-sm bg-white dark:bg-slate-900">
          <p className="text-slate-700 dark:text-slate-300 text-sm font-semibold">Late Penalties Collected</p>
          {loading ? (
            <div className="h-8 w-32 bg-slate-200 dark:bg-slate-700 animate-pulse rounded mt-1"></div>
          ) : (
            <b className="text-2xl block mt-1 text-slate-900 dark:text-slate-100">
      ...
      {/* Fee Scale Information - shown when we have circle data */}
