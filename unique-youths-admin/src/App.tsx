...
// Admin Pages - Direct imports (no lazy loading)
...
import PaymentClaims from "./pages/PaymentClaims";
    ...
    <BrowserRouter>
      <div className="min-h-screen bg-slate-100 dark:bg-slate-950">
        ...
        <header className="md:hidden sticky top-0 z-30 bg-[#173ea5] text-white p-4 flex items-center justify-between">
          ...
          <span className="font-bold text-sm">
            {(() => {
              ...
              const activeTab = TABS.find(t => (t.id === 'profit' && path === '/') || path === '/' + t.id);
              return activeTab?.label || (path === '/settings' ? 'Settings' : path === '/payment-claims' ? 'Payment Claims' : '');
            })()}
          </span>
        ...
        <main className="md:ml-64 p-4 sm:p-6 max-w-6xl">
          <Routes>
            <Route path="/" element={<ProfitMatrix token={token} refreshKey={refreshKey} />} />
            <Route path="/members" element={<Members token={token} refreshKey={refreshKey} />} />
            <Route path="/contributions" element={<ContributionsTracker token={token} refreshKey={refreshKey} />} />
            <Route path="/draw" element={<AjoRecipientDraw token={token} refreshKey={refreshKey} />} />
            <Route path="/slots" element={<MemberSlotGrid token={token} refreshKey={refreshKey} />} />
            <Route path="/disbursals" element={<MonthlyDisbursals token={token} refreshKey={refreshKey} />} />
            <Route path="/guarantors" element={<GuarantorPortal token={token} refreshKey={refreshKey} />} />
            <Route path="/broadcast" element={<BroadcastEngine token={token} refreshKey={refreshKey} />} />
            <Route path="/activity" element={<ActivityLog token={token} refreshKey={refreshKey} />} />
            <Route path="/settings" element={<Settings token={token} refreshKey={refreshKey} />} />
            <Route path="/payment-claims" element={<PaymentClaims token={token} refreshKey={refreshKey} />} />
          </Routes>
        </main>
      </div>
    </BrowserRouter>
...
function NavSidebar({ 
  ...
  token: string;
  refreshing: boolean;
  presence: { totalMembers: number; onlineNow: number };
  theme: Theme;
  ...
  const isTabActive = (id: TabId) => {
    if (id === 'profit') return location.pathname === '/';
    ...
    >
      ...
      <nav className="mt-6 space-y-1 flex-1 overflow-y-auto">
        {TABS.map(({ id, label, icon: Icon }) => (
          <Link
            ...
            className={`w-full flex items-center gap-3 p-3 rounded-lg text-left text-sm font-semibold transition ${
              isTabActive(id) ? "bg-red-600 text-white" : "text-blue-100 hover:bg-blue-900/40"
            }`}
          >
            <Icon size={18} />
            {label}
        ...
        <Link
          ...
          onClick={() => setNavOpen(false)}
          className={`w-full flex items-center gap-3 p-3 rounded-lg text-left text-sm font-semibold transition ${
            location.pathname === '/settings' ? "bg-red-600 text-white" : "text-blue-100 hover:bg-blue-900/40"
          }`}
        >
        ...
        <Link
          ...
          onClick={() => setNavOpen(false)}
          className={`w-full flex items-center gap-3 p-3 rounded-lg text-left text-sm font-semibold transition ${
            location.pathname === '/payment-claims' ? "bg-red-600 text-white" : "text-blue-100 hover:bg-blue-900/40"
          }`}
        >
